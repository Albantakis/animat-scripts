...
for t = 1:numtrials
    ...
    %             BigPhiMip_B(t,i) = mean(Phi_MIP);  
                ...
                MeanNumConcepts(count,i) = sum(results.p_LifeTransitions .*Num_Concepts);
                MeanSizeComplex(count,i) = sum(results.p_LifeTransitions .*subsys_numel);
                MeanHOConcepts(count,i) = sum(results.p_LifeTransitions .*MConceptOrder);
