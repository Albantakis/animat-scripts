...
for t = 1:numtrials
    ...
    %         end
            ...
            if exist(FilenameA,'file') == 2 
                ...
                BigPhiMip(count,i) = sum(results.p_LifeTransitions .* Phi_MIP);  
                BigPhiMip_B(count,i) = mean(Phi_MIP);  
                BigPhiMip_max(count,i) = max(Phi_MIP);  
...
save(File, 'evaluatedTrials','BigPhi','BigPhiMip','BigPhiMip_B','BigPhiMip_max','UsedNodes','Fitness_Phi_Corr', 'Fitness_level', 'MeanFitness', 'MeanNumConcepts', 'MeanHOConcepts', 'MeanSizeComplex', 'NumLifeStates', 'Num_Conn', 'cond', 'range');
