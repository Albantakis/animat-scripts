...
for i = 1:length(range)
    ...
    cd(strcat('Freeze_', cond, '_trial', int2str(trial)))
    Animat_gen = range(i);
...
%           subsys = results.Complex{LSindex(j)};
        ...
        BigPhi(i) = sum(results.p_LifeTransitions .* Phi); %weighted ave/expected value
        BigPhiMip(i) = sum(results.p_LifeTransitions .* Phi_MIP);  
        BigPhiMip_B(i) = mean(Phi_MIP);  
        BigPhiMip_max(i) = max(Phi_MIP);  
        
        diffStates(i) = length(unique(Phi_MIP));
        NumLifeStates(i) = size(results.p_LifeTransitions, 1);
